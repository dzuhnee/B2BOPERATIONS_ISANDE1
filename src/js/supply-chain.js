...
const sections = {
  ...
  launch: () => `<div class="welcome-row"><div><p class="eyebrow">FINAL SUPPLY APPROVAL</p><h1>Launch Confirmation</h1><p>Full supply confirmation remains unavailable until mandatory POS quantities are verified.</p></div></div><div class="settings-grid"><section class="panel"><div class="panel-heading"><div><h2>Pulilan, Bulacan</h2><p>Proposed opening August 1, 2026</p></div><span class="status good">92% Conditional</span></div><div class="settings-form toggles"><label><input type="checkbox" checked> Major equipment received and tested</label><label><input type="checkbox"> All 4 POS terminals received and verified</label><label><input type="checkbox" checked> Opening inventory substantially verified</label><label><input type="checkbox" checked> Packaging and smallwares on track</label><button class="primary-btn compact launch-confirm">Confirm Supply Readiness & Notify Store Manager</button><small>The missing POS terminals are mandatory; Pulilan remains conditionally ready.</small></div></section><section class="panel"><div class="panel-heading"><div><h2>Notification Preview</h2><p>Shared dependency shown to the Store Manager</p></div></div><div class="settings-form"><label>Recipient</label><input value="Ruth Torres — Store Manager" disabled><label>Subject</label><input value="Conditional Supply Readiness — Pulilan, Bulacan"><label>Message</label><textarea style="width:100%;min-height:170px;padding:12px;border:1px solid #ddd;border-radius:10px;font:inherit;font-size:12px">Pulilan is 92% supply-ready. Two of four mandatory POS terminals were received. Full supply readiness will be confirmed after the remaining two units are delivered and verified.</textarea></div></section></div>`,
