...
const readinessItems = [
  ...
  {id:4, requirement:'Certified crew assignment', category:'Staffing', owner:'Store Trainer', mandatory:true, evidence:'Certification roster', status:'Pending', remarks:'One service crew member is still awaiting final certification.'},
  ...
  {id:6, requirement:'Exterior signage installation', category:'Facility', owner:'Contractor', mandatory:true, evidence:'Completion photo', status:'Pending', remarks:'Installation scheduled for July 22.'}
];

const tasks = [
  {id:1, task:'Complete final POS transaction test', category:'Systems', owner:'IT Team', due:'Today', priority:'High', status:'In Progress'},
  {id:2, task:'Upload certified crew roster', category:'Staffing', owner:'Store Trainer', due:'Today', priority:'High', status:'Pending'},
...
function overviewTemplate(){
  ...
  return `${header('PASIG CAPITOL • LAUNCH PREPARATION','Good morning, Ruth!','Monitor branch readiness, resolve launch issues, and prepare the store for opening.','<div class="date-control">'+icon('calendar-days')+'<span>July 21, 2026</span></div>')}
  ...
  <div class="dashboard-grid">
    <section class="panel branch-panel"><div class="panel-heading"><div><h2>Launch Timeline</h2><p>Current progress toward opening approval</p></div><button class="text-btn" data-open-section="clearance">View clearance ${icon('arrow-up-right')}</button></div><div class="branch-list">
      ${['Site turnover|Complete','Equipment installation|Complete','Certified staff assignment|Pending','Final facility verification|In Progress','Opening clearance|'+clearanceStatus].map((entry,index)=>{const [name,status]=entry.split('|');return `<article class="branch-row" data-search="${name} ${status}"><div class="branch-rank">${status==='Complete'?icon('check'):index+1}</div><div class="branch-name"><strong>${name}</strong><span>${status==='Complete'?'Verified and recorded':'Requires follow-up before opening'}</span></div><span class="status ${statusClass(status)}">${status}</span></article>`}).join('')}
    </div></section>
...
function clearanceTemplate(){ const allowed=canRequestClearance(); return `${header('FINAL APPROVAL','Opening Clearance','Review the readiness summary and submit the branch for opening approval.','')}
